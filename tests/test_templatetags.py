...
class TemplateTagsTest(HostsTestCase):
    ...
    def test_host_url_tag_simple(self):
        self.assertRender(
            "{% host_url 'simple-direct' host 'www' %}", "//www.example.com/simple/"
        )
        self.assertRender(
            "{% host_url 'simple-direct' host 'www' as "
    ...
    def test_url_tag_override(self):
        with self.settings(
            ...
            self.assertRender(
                "{% url 'simple-direct' host 'www' %}", "//www.example.com/simple/"
            )
            self.assertRender(
    ...
    def test_host_url_tag_with_scheme(self):
        self.assertRender(
            "{% host_url 'simple-direct' scheme 'http' %}",
            "http://www.example.com/simple/",
        )
        self.assertRender(
            "{% host_url 'simple-direct' scheme 'git' %}",
    ...
    def test_host_url_tag_with_port(self):
        self.assertRender(
            "{% host_url 'simple-direct' port '8000' %}",
    ...
    def test_host_url_tag_with_args(self):
        ...
        self.assertRender(
            "{% host_url 'simple-direct' as yeah "
            "host 'with_args' 'www.eggs.spam' %}{{ yeah }}",
            "//www.eggs.spam/simple/",
        )

    @override_settings(
        DEFAULT_HOST="www", PARENT_HOST="eggs.spam", ROOT_HOSTCONF="tests.hosts.simple"
    )
    ...
    @override_settings(
        DEFAULT_HOST="www", ROOT_HOSTCONF="tests.hosts.simple", PARENT_HOST="eggs.spam"
    )
    def test_host_url_tag_parent_host(self):
        self.assertRender(
            "{% host_url 'simple-direct' host 'static' %}", "//static.eggs.spam/simple/"
        )
    ...
    def test_host_url_no_www(self):
        self.assertRender(
            "{% host_url 'simple-direct' host 'without_www' %}", "//example.com/simple/"
        )
    ...
    def test_raises_template_syntaxerror(self):
        ...
        self.assertRaises(
            TemplateSyntaxError, self.render, "{% host_url 'simple-direct' host %}"
        )
        self.assertRaises(
            TemplateSyntaxError, self.render, "{% host_url 'simple-direct' as %}"
        )
        self.assertRaises(
            TemplateSyntaxError, self.render, "{% host_url simple-direct %}"
        )
        self.assertRaises(
            TemplateSyntaxError,
            parse_params,
        ...
        self.assertRaises(
            TemplateSyntaxError, parse_params, "host_url", Parser([""]), "\n='johndoe'"
        )
