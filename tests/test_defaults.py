...
class PatternsTests(HostsTestCase):
    def test_pattern(self):
        ...
        self.assertEqual(repr(host_patterns[0]), '<host api: regex="api" urlconf="api.urls" scheme="//" port="">')
...
class HostTests(HostsTestCase):
    ...
    def test_host_string_callback(self):
        api_host = host(r"api", "api.urls", name="api", callback="django_hosts.resolvers.get_host_patterns")
    ...
    def test_host_nonexistent_callback(self):
        ...
        self.assertRaisesMessage(
            ImproperlyConfigured, "Could not import 'whatever'. Error was: No module named", lambda: api_host.callback
        )
        ...
        api_host = host(r"api", "api.urls", name="api", callback="tests.broken_module.yeah_yeah")
