...
class ReverseTest(HostsTestCase):
    ...
    def test_reverse_host(self):
        self.assertRaises(
            ValueError, reverse_host, "with_kwargs", ["spam"], dict(eggs="spam")
        )
        ...
        self.assertEqual(
            "johndoe", reverse_host("with_kwargs", None, dict(username="johndoe"))
        )
    ...
    def test_reverse_custom_port(self):
        self.assertEqual(
            reverse("simple-direct", host="port"), "//port.spam.eggs:12345/simple/"
        )
        self.assertEqual(
...
class UtilityTests(HostsTestCase):
    ...
    def test_missing_host_patterns(self):
        self.assertRaisesMessage(
            ImproperlyConfigured, "Missing ROOT_HOSTCONF setting", get_host_patterns
        )
